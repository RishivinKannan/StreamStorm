...
class StormControlsClass {
    ...
    log_analytics(stormData) {
        if (stormData.subscribe) {
            logEvent(analytics, 'subscribe');
        }
        if (stormData.subscribe_and_wait) {
            logEvent(analytics, 'subscribe_and_wait');
        }
        if (stormData.subscribe_and_wait_time) {
            logEvent(analytics, 'subscribe_and_wait_time', {
                time: stormData.subscribe_and_wait_time,
            });
        }
        if (stormData.slow_mode != 5) {
            logEvent(analytics, 'slow_mode_change', {
                time: stormData.slow_mode,
            });
        }
        if (stormData.background) {
            logEvent(analytics, 'background_load');
        }
        logEvent(analytics, 'channel_count', {
            count: stormData.channels.length,
        });
        logEvent(analytics, 'channel_selection_mode', {
            mode: stormData.channelSelection,
        });
    }
        ...
        fetch(`${this.hostAddress}/storm/start`, {
            ...
            .catch((error) => {
                this.notifications.show(
                    error.message ||
                        'An error occurred while starting the storm',
                    {
                        severity: 'error',
                    }
                );
    ...
    stopStorm(setStormInProgress) {
        ...
        fetch(`${this.hostAddress}/storm/stop`, {
            ...
            .catch((error) => {
                this.notifications.show(
                    'An error occurred while stopping the storm',
                    {
                        severity: 'error',
                    }
                );
    ...
    pauseStorm() {
        ...
        fetch(`${this.hostAddress}/storm/pause`, {
            ...
            .catch((error) => {
                this.notifications.show(
                    'An error occurred while pausing the storm',
                    {
                        severity: 'error',
                    }
                );
    ...
    resumeStorm() {
        ...
        fetch(`${this.hostAddress}/storm/resume`, {
            ...
            .catch((error) => {
                this.notifications.show(
                    'An error occurred while resuming the storm',
                    {
                        severity: 'error',
                    }
                );
    ...
    dontWait() {
        ...
        fetch(`${this.hostAddress}/storm/start_storm_dont_wait`, {
            ...
            .then((data) => {
                if (data.success) {
                    this.notifications.show(
                        'Each channel will start storming without waiting for others!',
                        {
                            severity: 'success',
                        }
                    );
                    ...
                    this.notifications.show(
                        data.message || 'Failed to set dont-wait',
                        {
                            severity: 'error',
                        }
                    );
            ...
            .catch((error) => {
                this.notifications.show(
                    'An error occurred while setting dont-wait',
                    {
                        severity: 'error',
                    }
                );
        ...
        fetch(`${this.hostAddress}/storm/change_messages`, {
            ...
            .then((data) => {
                if (data.success) {
                    ...
                    this.notifications.show(
                        data.message || 'Failed to change messages',
                        {
                            severity: 'error',
                        }
                    );
            ...
            .catch((error) => {
                this.notifications.show(
                    'An error occurred while changing messages',
                    {
                        severity: 'error',
                    }
                );
        ...
        fetch(`${this.hostAddress}/storm/change_slow_mode`, {
            ...
            .then((data) => {
                if (data.success) {
                    ...
                    this.notifications.show(
                        data.message || 'Failed to change slow mode',
                        {
                            severity: 'error',
                        }
                    );
            ...
            .catch((error) => {
                this.notifications.show(
                    'An error occurred while changing slow mode',
                    {
                        severity: 'error',
                    }
                );
    ...
    startMoreChannels(channels) {
        ...
        fetch(`${this.hostAddress}/storm/start_more_channels`, {
            ...
            .then((data) => {
                if (data.success) {
                    this.notifications.show(
                        'More channels started successfully!',
                        {
                            severity: 'success',
                        }
                    );
                    ...
                    this.notifications.show(
                        data.message || 'Failed to start more channels',
                        {
                            severity: 'error',
                        }
                    );
            ...
            .catch((error) => {
                this.notifications.show(
                    'An error occurred while starting more channels',
                    {
                        severity: 'error',
                    }
                );
