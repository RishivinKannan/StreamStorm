        ...
        fetch(`${hostAddress}/environment/profiles/create`, {
            ...
            .then((data) => {
                if (data.success) {
                    ...
                    logEvent(analytics, 'create_profiles_success', {
                        count: profiles,
                    });
                } else {
                    setErrorText(
                        data.message ||
                            'An error occurred while creating profiles.'
                    );
                    notifications.show('Failed to create profiles.', {
                        severity: 'error',
                    });
                    logEvent(analytics, 'create_profiles_failed', {
                        count: profiles,
                    });
                }
            })
            .catch((error) => {
                setErrorText(
                    'An error occurred while creating profiles. Try again.'
                );
                notifications.show('Failed to create profiles.', {
                    severity: 'error',
                });
                atatus.notify(error, {}, ['create_profiles_error']);
                logEvent(analytics, 'create_profiles_error', {
                    count: profiles,
                });
            })
            .finally(() => {
        ...
        <div className="create-profiles-container">
            <div className="section-header">
                <UserPlus
                    ...
                    size={20}
                    color={'var(--input-active-red-dark)'}
                />
                <h3 className={`section-title ${colorScheme}-text`}>
                    Create / Fix Profiles
                </h3>
            </div>
